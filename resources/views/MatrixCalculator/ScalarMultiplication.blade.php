...
<body>
    <main>
        @if ($Email)
         ...
         <center><h2 style=" color: rgb(236, 242, 236); font-size: 101%;">Scalar Multiplication</h2>
            <hr>
   <div class="container">
        <form id="matrixForm" action="{{url('')}}/MatrixCalculator/ScalarMultiplication" method="POST">
            @csrf
...
@enderror
            <p></p>
             <input type="number" name="scalar" id="scalar" placeholder="Enter scalar" required>
              @error('scalar')
            <br>
    <center><span style="font-size: 15.9px; color: red;">{{$message}}</span></center>
@enderror
...
@isset($matrix)
 <div class="container">
        ...
        <h2>Scalar Number: <center><span style='font-size: 19px; color: white;'>{{$Scalar}}</span></center></h2>
        <h2>Scalar Multiplication:</h2>
        <h2 style="font-size: 15.9px; color: yellow;">Matrix A</h2>
        <table class="answer">
            @foreach ($scalarMatrix as $row)
                <tr>
                    @foreach ($row as $value)
                        <td align="center">{{ $value }}</td>
                    @endforeach
