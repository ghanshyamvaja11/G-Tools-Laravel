...
<main>
    ...
    <center><h2 style=" color: rgb(236, 242, 236); font-size: 101%;">Determinant</h2>
    <div class="container">
        <form id="matrixForm" action="{{url('')}}/MatrixCalculator/Determinant" method="POST">
            @csrf
            <h2>Matrix A</h2>
            <input type="number" name="rows" id="rows" placeholder="Enter number Of Rows" required>
            <br>
            @error('rows')
                <center><span style="font-size: 19px; color: red">{{$message}}</span></center>
            @enderror
            <input type="number" name="columns" id="columns" placeholder="Enter number of Columns" required><br>
             @error('columns')
                <center><span style="font-size: 19px; color: red">{{$message}}</span></center>
            @enderror
            <div id="A"></div><p></p>
    ...
    <script>
        ...
        {
            rowsInput.value = columnsInput.value;
    ...
    @isset($matrix)
    <div class="container">
        <h2 style="color: yellow;">Matrix A</h2>
        @if (isset($matrix))
        <table class="answer">
        ...
        <h2 style="color: yellow;">Determinant:</h2>
                <span style="color: white;" align="center"> ▲ = {{ $determinant }}</span>
            </tr>
        </table>
    </div>
    </main>
